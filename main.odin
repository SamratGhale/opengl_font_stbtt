...
main :: proc(){
    ...
    program_id, ok := gl.load_shaders_file("vert.glsl", "frag.glsl")

    if !ok{
	error, type := gl.get_last_error_message()
	fmt.println(error)
	fmt.println(type)
	return
    }
    ...
    rect_buffer_filled : i32 = 0

    rect_buffer : [255]rect_instance

    rect_instances_vbo : u32 = 0
    ...
    glyph_atlas_width  : i32 = 512
    glyph_atlas_height : i32 = 512

    glyph_atlas_texture : u32  = 0
